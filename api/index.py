...
def call_dify_api(user_id, image_bytes):
    ...
    try:
        with requests.post(dify_api_url, headers=headers, files=files, data=data, stream=False, timeout=300) as response:
            response.raise_for_status()
            return response.text # Dify 在 blocking mode 下會直接回傳文字
